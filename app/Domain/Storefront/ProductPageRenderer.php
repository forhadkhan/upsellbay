...
final class ProductPageRenderer extends AbstractOfferRenderer {
	...
	public function render_offer( array $offer, array $context = array() ): string {
		...
		$button_text  = '' !== $button_text ? $button_text : __( 'Add with product', 'upsellbay' );
		...
		$classes = array(
			'upsellbay-offer',
			'upsellbay-offer--product_upsell',
			'upsellbay-offer--product-addon',
		);
		if ( '' !== $image_html ) {
		...
		return \WPAnchorBay\UpsellBay\Core\Hooks::filter( 'render_offer_html', $html, $offer, 'product_upsell', $context );
	}
}
