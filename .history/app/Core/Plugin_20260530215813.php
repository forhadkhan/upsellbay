...
final class Plugin {
	...
	public function handle_activate_license(): void {
		...
		if ( is_wp_error( $result ) ) {
			$redirect_url = add_query_arg( 'wc_error', rawurlencode( $result->get_error_message() ), $redirect_url );
		} else {
			$redirect_url = add_query_arg( 'wc_message', rawurlencode( __( 'License activated successfully.', 'upsellbay' ) ), $redirect_url );
		}
	...
	public function handle_remove_license(): void {
		...
		$redirect_url = add_query_arg(
			'wc_message',
			rawurlencode( __( 'Local license data removed.', 'upsellbay' ) ),
	...
	public function handle_check_license(): void {
		...
		if ( $is_valid ) {
			$redirect_url = add_query_arg( 'wc_message', rawurlencode( __( 'License check complete: valid.', 'upsellbay' ) ), $redirect_url );
		} else {
			$redirect_url = add_query_arg( 'wc_error', rawurlencode( __( 'License check failed: inactive.', 'upsellbay' ) ), $redirect_url );
		}
	...
	/**
	 * Render a sitewide admin banner when the license is missing, wrong, or failed.
	 *
	 * Shows on every admin page so the problem is always visible until resolved.
	 *
	 * @since 1.0.0
	 *
	 * @return void
	...
	public function render_license_banner(): void {
		...
		// Show on all admin screens except WooCommerce onboarding and AJAX requests.
		if ( 'woocommerce_onboarding' === $screen->id ) {
			return;
		}
		...
		// Active or dev-mode license needs no banner.
		if ( in_array( $status_code, array( 'active', 'dev' ), true ) ) {
			return;
		}

		$settings_url = admin_url( 'admin.php?page=upsellbay&tab=settings' );
		$settings_license_url = admin_url( 'admin.php?page=upsellbay&tab=settings#license' );
		...
		printf(
			'<div class="notice notice-warning is-dismissible"><p><strong>%s</strong> %s <a href="%s" class="button button-small">%s</a></p></div>',
