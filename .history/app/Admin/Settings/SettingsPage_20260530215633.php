...
final class SettingsPage {
	 ...
	 * @param LicenseClient|null                   $license_client License client.
	 ...
	 */
	public function __construct( Settings $settings, ?LicenseClient $license_client = null, ?callable $can_manage = null, ?callable $verify_nonce = null, ?array $sections = null ) {
	...
	private function render_license_section(): void {
		...
		$is_active      = in_array( $status_code, array( 'active', 'dev' ), true );
		...
		$activate_nonce = wp_create_nonce( 'upsellbay_activate_license' );
		$activate_url   = admin_url( 'admin-post.php?action=upsellbay_activate_license' );
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo '<tr><th scope="row"><label for="upsellbay_new_license_key">' . esc_html__( 'New License Key', 'upsellbay' ) . '</label> ' . $this->help_tip( __( 'Paste a new UpsellBay license key here to activate or replace the current key.', 'upsellbay' ) ) . '</th>';
		echo '<td>';
		echo '<form method="post" action="' . esc_url( $activate_url ) . '" style="display:flex;gap:8px;align-items:center;">';
		echo '<input type="hidden" name="_wpnonce" value="' . esc_attr( $activate_nonce ) . '" />';
		echo '<input type="text" name="upsellbay_license_key" id="upsellbay_new_license_key" value="" placeholder="' . esc_attr__( 'Enter license key', 'upsellbay' ) . '" class="regular-text" autocomplete="off" style="flex:1;" />';
		echo '<button type="submit" class="button button-primary" style="margin-left:5px;">' . esc_html__( 'Activate License', 'upsellbay' ) . '</button>';
		echo '</form>';
		echo '</td></tr>';
	}
