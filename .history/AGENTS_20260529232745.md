...
## Overlay and Metadata Notes

This repository may use an orphan overlay strategy for dot-prefixed directories and project metadata. Treat `.meta/`, `.agents/`, `.history/`, `.codex/`, and root agent guides as metadata assets. If you are asked to commit metadata changes, verify the repository's current branch strategy before staging so app code and overlay assets do not get mixed accidentally.
